...
void Mat::setVal(int a, int b, ll128 v)
{
    val[b * r + a] = v;
}

void Mat::set_val(vector<ll128> val)
{
    this->val.clear();
...
/**
 * @brief modify val from begin using vector val
 *
 * @param begin
 * @param val
 */
void Mat::insert_val(int begin, vector<ll128> val)
{
    this->val.insert(this->val.begin() + begin, val.begin(), val.end());
}
...
// FIXME: This has a tiny bug that the parameter `ed` is not used. That is, the whose val is appended, with no size limit.
void Mat::append(int st, int ed, Mat *a)
{
    copy(val.begin(), val.end(), a->val.begin() + st);
}

void Mat::append_with_size(int st, int size, Mat *a)
{
    copy(val.begin(), val.begin() + size, a->val.begin() + st);
}
...
Mat Mat::random_permutation_matrix(int r, int c)
{
    if (r != c)
        DBGprint("Row not match Col for permutation matrix.");
    int raw[r];
    for (int i = 0; i < r; i++)
    {
        raw[i] = i;
    }
    random_shuffle(raw, raw + r);
    Mat ret(r, c);
    for (int i = 0; i < r; i++)
    {
        ret(i, raw[i]) = 1;
...
std::vector<ll128> Mat::to_vector()
{
    return val;
}