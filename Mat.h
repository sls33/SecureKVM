...
public:
    ...
    void setVal(int a, int b, ll128 v);
    void set_val(vector<ll128> val);
    void insert_val(int begin, vector<ll128> val);
    int rows() const;
    int cols() const;
    int size() const;
    ...
    bool operator==(Mat p);
    Mat transpose() const;
    Mat reduce_sum_x() const;
    Mat reduce_sum_y() const;
    Mat operator+(const Mat &a);
    ...
    void append_with_size(int st, int size, Mat *a);
    Mat mod(ll128 b);
    int count();
    ...
    static Mat random_permutation_matrix(int r, int c);

    std::vector<ll128> to_vector();
};
