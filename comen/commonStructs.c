...
void struct_nGrams_allocate (nGram_struct * nGrams, alphabet_struct * alphabet)
{
  ...
  (nGrams->cP) = (PROB_TYPE *) realloc ((nGrams->cP), sizeof (PROB_TYPE) * (nGrams->sizeOf_cP));
  (nGrams->iP) = (PROB_TYPE *) realloc ((nGrams->iP), sizeof (PROB_TYPE) * (nGrams->sizeOf_iP));
  (nGrams->eP) = (PROB_TYPE *) realloc ((nGrams->eP), sizeof (PROB_TYPE) * (nGrams->sizeOf_eP));
  (nGrams->len) = (PROB_TYPE *) realloc ((nGrams->len), sizeof (PROB_TYPE) * (nGrams->sizeOf_len));
...
void struct_nGrams_copyArrays (nGram_struct * dest, nGram_struct * src)
{
  ...
  (dest->cP) = (PROB_TYPE *) realloc ((dest->cP), sizeof (PROB_TYPE) * (src->sizeOf_cP));
  (dest->iP) = (PROB_TYPE *) realloc ((dest->iP), sizeof (PROB_TYPE) * (src->sizeOf_iP));
  (dest->eP) = (PROB_TYPE *) realloc ((dest->eP), sizeof (PROB_TYPE) * (src->sizeOf_eP));
  (dest->len) = (PROB_TYPE *) realloc ((dest->len), sizeof (PROB_TYPE) * (src->sizeOf_len));
  ...
  // copy content
  memcpy ((dest->cP), (src->cP), sizeof (PROB_TYPE) * (src->sizeOf_cP));
  memcpy ((dest->iP), (src->iP), sizeof (PROB_TYPE) * (src->sizeOf_iP));
  memcpy ((dest->eP), (src->eP), sizeof (PROB_TYPE) * (src->sizeOf_eP));
  memcpy ((dest->len), (src->len), sizeof (PROB_TYPE) * (src->sizeOf_len));
