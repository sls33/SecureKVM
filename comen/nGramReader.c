...
bool read_level (PROB_TYPE *levelArray, const int sizeOf_Array, FILE * fp)
{
  char curLine[MAX_LINE_LENGTH];  // current line
  PROB_TYPE curValue;                 // current value
  ...
  for (size_t i = 0; i < sizeOf_Array; i++)
  {
    ...
    {                           // reading successfull
      ...
      sscanf (curLine, "%lf", &curValue);   // modify from %d to %lf to load double values. Note lf is necessary!!
