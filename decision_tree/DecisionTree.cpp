...
public:
    ...
    void run(Table table, int nodeIndex) {
        ...
        for(int i=0;i< initialTable.attrValueList[selectedAttrIndex].size(); i++) {
            ...
            for(int j=0;j<candi.size(); j++) {
                nextTable.data.push_back(table.data[candi[j]]);
            }
    ...
    double getEntropy(Table table) {
        ...
        int total_count = (int)table.data.size();
        map<string, int> labelCount;
    ...
    double getGain(Table table, int attrIndex) {
        return getEntropy(table) - getInfoAttrD(table, attrIndex);
    }
